        ...
        <div className="min-h-screen bg-gradient-to-b from-blue-700 to-green-900 text-white relative">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 p-4 sm:p-8">
                {usersWithSkills.map((user) => (
                    <Card
                        key={user.id}
                        className="bg-white text-gray-900 p-4 shadow-lg rounded-lg cursor-pointer"
                        onClick={() => openViewModal(user)} // Open modal on click
                    >
            ...
            <Dialog open={isViewModalOpen} onOpenChange={setIsViewModalOpen}>
                <DialogContent className="sm:max-w-[600px]">
                    <DialogHeader>
                        <DialogTitle>{selectedCardData?.username}</DialogTitle>
                    </DialogHeader>
                    {selectedCardData && (
                        <div>
                            <img
                                src={selectedCardData.avatarUrls?.[0] || "https://via.placeholder.com/150"}
                                alt="User Avatar"
                                className="w-full h-40 object-cover rounded-lg"
                            />
                            <p>Email: {selectedCardData.email}</p>
                            <p>Role: {selectedCardData.role}</p>
                            <h3 className="mt-4">Skills:</h3>
                            <ul>
                                {selectedCardData.skills.map(skill => (
                                    <li key={skill.id}>
                                        {skill.title}: {skill.description} (${skill.price})
                                    </li>
                                ))}
                            </ul>
                        </div>
